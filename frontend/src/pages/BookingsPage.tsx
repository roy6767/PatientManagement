...
const statusConfig = {
  BOOKED:    { label: 'Booked',    bg: '#0d1f2e', color: '#60a5fa', icon: Clock },
  COMPLETED: { label: 'Completed', bg: '#1e4a7a', color: '#60a5fa', icon: CheckCircle },
...
export default function BookingsPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-5">
      <div>
        <h2 className="text-lg font-bold" style={{ color: '#e0eaff' }}>Bookings</h2>
        <p className="text-sm mt-0.5" style={{ color: '#5a7fa8' }}>Look up appointments by patient ID</p>
      </div>
        ...
        {!queried ? (
          <div className="flex flex-col items-center py-16 px-6 text-center">
            <div className="w-14 h-14 flex items-center justify-center mb-3" style={{ backgroundColor: '#0f1c2e' }}>
              <CalendarDays size={26} style={{ color: '#3a5a8a' }} />
            </div>
            <p className="text-sm font-semibold text-white">No search yet</p>
            <p className="text-xs mt-1" style={{ color: '#5a7fa8' }}>Enter a patient UUID above to load their bookings.</p>
          </div>
        ) : loading ? (
          ...
          <div className="flex flex-col items-center py-16 px-6 text-center">
            ...
            <p className="text-xs mt-1" style={{ color: '#5a7fa8' }}>This patient has no appointments on record.</p>
          </div>
        ) : (
          <>
            <div className="px-6 py-4" style={{ borderBottom: '1px solid #1e3a6a', backgroundColor: '#0f1c2e' }}>
              <p className="text-sm font-semibold" style={{ color: '#e0eaff' }}>
                  ...
                  <div className="flex items-start justify-between gap-6 flex-wrap">
                    <div className="space-y-2">
                      <div className="flex items-center gap-2 flex-wrap">
                        <CalendarDays size={15} style={{ color: '#5a7fa8' }} />
                        <span className="text-sm font-semibold text-white">{b.appointmentDate}</span>
                        <span className="text-xs" style={{ color: '#5a7fa8' }}>{b.startTime} – {b.endTime}</span>
                      </div>
                      ...
                      {b.status === 'BOOKED' && (
                        <div className="flex gap-2">
                          <button onClick={async () => { await completeBooking(b.id); fetchByPatient(patientId) }}
                            ...
                            style={{ border: '1px solid #1e4a7a', color: '#60a5fa' }}>Complete</button>
