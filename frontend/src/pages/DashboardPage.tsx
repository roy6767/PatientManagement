  ...
  return (
    <div className="max-w-4xl mx-auto space-y-5">
      ...
      <div className="relative px-8 py-7" style={{ background: 'linear-gradient(135deg,#1e4a7a 0%,#0f2a4d 100%)', border: '1px solid #1e4a7a' }}>
        <p className="text-sm" style={{ color: '#7b96c2' }}>{getGreeting()},</p>
        <h2 className="text-2xl font-bold text-white mt-0.5 capitalize">{firstName}</h2>
        <p className="text-sm mt-1" style={{ color: '#7b96c2' }}>Here's what's happening in your system today.</p>
        <div className="mt-4 inline-flex items-center gap-2 px-3 py-1" style={{ backgroundColor: '#1e3060', border: '1px solid #1e4a7a' }}>
          ...
          <span className="text-xs" style={{ color: '#7b96c2' }}>All systems operational</span>
        </div>
      </div>
        ...
        {stats.map(({ label, value, icon: Icon, to, trend }) => (
          <Link key={label} to={to} className="group block p-6 transition-all" style={{ backgroundColor: '#162040', border: '1px solid #1e3a6a' }}>
            <div className="flex items-start justify-between mb-5">
              ...
              <ArrowRight size={15} style={{ color: '#5a7fa8' }} className="mt-0.5" />
            </div>
            <p className="text-3xl font-bold leading-none" style={{ color: '#93c5fd' }}>{value}</p>
            <p className="text-sm font-semibold mt-2 text-white">{label}</p>
            <p className="text-xs mt-1 flex items-center gap-1" style={{ color: '#5a7fa8' }}>
      ...
      <div style={{ backgroundColor: '#162040', border: '1px solid #1e3a6a' }}>
        <div className="flex items-start justify-between px-6 py-5" style={{ borderBottom: '1px solid #1e3a6a' }}>
          <div>
            <h3 className="text-sm font-semibold" style={{ color: '#e0eaff' }}>Recent Patients</h3>
            <p className="text-xs mt-0.5" style={{ color: '#5a7fa8' }}>Latest registered patients</p>
          </div>
          <Link to="/patients" className="text-xs font-semibold flex items-center gap-1" style={{ color: '#60a5fa' }}>
        ...
        {patients.length === 0 ? (
          <div className="flex flex-col items-center py-12 px-6 text-center">
            <div className="w-12 h-12 flex items-center justify-center mb-3" style={{ backgroundColor: '#0f1c2e' }}>
              <Users size={22} style={{ color: '#3a5a8a' }} />
            </div>
            <p className="text-sm font-semibold text-white">No patients yet</p>
            <p className="text-xs mt-1" style={{ color: '#5a7fa8' }}>Patients will appear here once added.</p>
          ...
          patients.map((p, i) => (
            <div key={p.id} className="flex items-center gap-4 px-6 py-4"
              ...
              <span className="shrink-0 text-xs font-medium px-2.5 py-1" style={{ backgroundColor: '#0f1c2e', color: '#60a5fa' }}>
                {p.gender}
              </span>
            </div>
          ))
        ...
        {[
          ...
          <Link key={to} to={to} className="flex items-center gap-4 px-6 py-5" style={{ backgroundColor: '#162040', border: '1px solid #1e3a6a' }}>
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs mt-0.5" style={{ color: '#5a7fa8' }}>{sub}</p>
            </div>
            <ArrowRight size={15} style={{ color: '#5a7fa8' }} className="shrink-0" />
          </Link>
        ))}
      </div>
    </div>
