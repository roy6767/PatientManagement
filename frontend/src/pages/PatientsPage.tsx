  ...
  return (
    <div className="max-w-4xl mx-auto space-y-5">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h2 className="text-lg font-bold" style={{ color: '#e0eaff' }}>Patients</h2>
          <p className="text-sm mt-0.5" style={{ color: '#5a7fa8' }}>{total} total registered patients</p>
        </div>
      ...
      <div className="relative max-w-xs">
        <Search size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 pointer-events-none" style={{ color: '#5a7fa8' }} />
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center py-16 px-6 text-center">
            <div className="w-14 h-14 flex items-center justify-center mb-3" style={{ backgroundColor: '#0f1c2e' }}>
              <Users size={26} style={{ color: '#3a5a8a' }} />
            </div>
            <p className="text-sm font-semibold text-white">No patients found</p>
            <p className="text-xs mt-1" style={{ color: '#5a7fa8' }}>{search ? 'Try a different search term.' : 'Add your first patient.'}</p>
          </div>
        ) : (
          <>
            <div className="grid grid-cols-12 px-6 py-3" style={{ backgroundColor: '#0f1c2e', borderBottom: '1px solid #1e3a6a' }}>
              <div className="col-span-5 text-xs font-bold uppercase tracking-wide" style={{ color: '#5a7fa8' }}>Patient</div>
              <div className="col-span-4 text-xs font-bold uppercase tracking-wide hidden md:block" style={{ color: '#5a7fa8' }}>Phone</div>
              <div className="col-span-3 text-xs font-bold uppercase tracking-wide" style={{ color: '#5a7fa8' }}>Gender</div>
            </div>
            {patients.map((p, i) => (
              <div key={p.id} className="grid grid-cols-12 items-center px-6 py-4"
                ...
                <div className="col-span-5 flex items-center gap-3 min-w-0 pr-3">
                  <div className="w-9 h-9 flex items-center justify-center font-bold text-sm shrink-0" style={{ backgroundColor: '#1a2d52', color: '#93c5fd' }}>
                    {p.name[0].toUpperCase()}
                  </div>
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-white truncate">{p.name}</p>
                    <p className="text-xs truncate" style={{ color: '#5a7fa8' }}>{p.email}</p>
                  </div>
                </div>
                <div className="col-span-4 text-sm hidden md:block pr-3" style={{ color: '#7b96c2' }}>{p.phoneNumber || '—'}</div>
                <div className="col-span-3">
                  <span className="text-xs font-semibold px-2.5 py-1" style={{ backgroundColor: '#0f1c2e', color: '#60a5fa' }}>{p.gender}</span>
                </div>
              </div>
            ))}
            {totalPages > 1 && (
              <div className="flex items-center justify-between px-6 py-3.5" style={{ borderTop: '1px solid #1e3a6a', backgroundColor: '#0f1c2e' }}>
                <p className="text-xs" style={{ color: '#5a7fa8' }}>
                  Showing <span className="font-semibold" style={{ color: '#60a5fa' }}>{page * pageSize + 1}–{Math.min((page+1)*pageSize, total)}</span> of <span className="font-semibold" style={{ color: '#60a5fa' }}>{total}</span>
                </p>
                <div className="flex items-center gap-1.5">
                  <button onClick={() => setPage(p => Math.max(0, p-1))} disabled={page === 0}
                    className="w-8 h-8 flex items-center justify-center transition disabled:opacity-30"
                    style={{ border: '1px solid #1e3a6a', color: '#60a5fa' }}>
                    <ChevronLeft size={15} />
                  </button>
                  <span className="text-xs px-1" style={{ color: '#5a7fa8' }}>{page+1} / {totalPages}</span>
