...
export default function DepartmentsPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-5">
      <div>
        <h2 className="text-lg font-bold" style={{ color: '#e0eaff' }}>Departments</h2>
        <p className="text-sm mt-0.5" style={{ color: '#5a7fa8' }}>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center py-16 px-6 text-center" style={{ backgroundColor: '#162040', border: '1px solid #1e3a6a' }}>
          <div className="w-14 h-14 flex items-center justify-center mb-3" style={{ backgroundColor: '#0f1c2e' }}>
            <Building2 size={26} style={{ color: '#3a5a8a' }} />
          </div>
          <p className="text-sm font-semibold text-white">No departments found</p>
          <p className="text-xs mt-1" style={{ color: '#5a7fa8' }}>No departments are currently in the system.</p>
        </div>
      ) : (
            ...
            <div key={dept.id} style={{ backgroundColor: '#162040', border: '1px solid #1e3a6a' }}>
              <button onClick={() => toggle(dept.id)} className="w-full flex items-center gap-4 px-6 py-4 text-left">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs mt-0.5 truncate" style={{ color: '#5a7fa8' }}>{dept.description || 'No description'}</p>
                </div>
                <div className="flex items-center gap-3 shrink-0">
                  <span className="text-xs font-semibold px-2.5 py-1"
                    style={dept.active ? { backgroundColor: '#1e4a7a', color: '#60a5fa' } : { backgroundColor: '#1a1a1a', color: '#6b7280' }}>
                    {dept.active ? 'Active' : 'Inactive'}
                  </span>
                  <div className="w-7 h-7 flex items-center justify-center" style={{ backgroundColor: '#0f1c2e', color: '#5a7fa8' }}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        <div className="flex items-center gap-2 mb-3">
                          ...
                          <p className="text-xs font-bold uppercase tracking-wider" style={{ color: '#5a7fa8' }}>Doctors ({doctors.length})</p>
                        </div>
                        {doctors.length === 0 ? (
                          <p className="text-xs italic pl-1" style={{ color: '#5a7fa8' }}>No doctors assigned.</p>
                        ) : doctors.map(d => (
                          <div key={d.id} className="flex items-center gap-3 px-3 py-2.5 mb-2" style={{ backgroundColor: '#0f1c2e' }}>
                            <div className="w-8 h-8 flex items-center justify-center font-bold text-xs shrink-0" style={{ backgroundColor: '#1a2d52', color: '#93c5fd' }}>
                              {d.firstName[0]}
                            </div>
                            <div className="flex-1 min-w-0">
                              <p className="text-xs font-semibold text-white truncate">Dr. {d.firstName} {d.lastName}</p>
                              <p className="text-[11px] truncate" style={{ color: '#5a7fa8' }}>{d.designation}</p>
                            </div>
                            <span className="text-[10px] font-semibold px-2 py-0.5 shrink-0"
                              style={d.active ? { backgroundColor: '#1e4a7a', color: '#60a5fa' } : { backgroundColor: '#1a1a1a', color: '#6b7280' }}>
                              {d.active ? 'Active' : 'Off'}
                            </span>
                          </div>
                        ))}
                      ...
                      <div>
                        <div className="flex items-center gap-2 mb-3">
                          ...
                          <p className="text-xs font-bold uppercase tracking-wider" style={{ color: '#5a7fa8' }}>Treatments ({treatments.length})</p>
                        </div>
                        {treatments.length === 0 ? (
                          <p className="text-xs italic pl-1" style={{ color: '#5a7fa8' }}>No treatments listed.</p>
                        ) : treatments.map(t => (
                          <div key={t.id} className="flex items-center justify-between px-3 py-2.5 mb-2" style={{ backgroundColor: '#0f1c2e' }}>
                            ...
                            <span className="text-xs font-bold px-2.5 py-1 shrink-0" style={{ backgroundColor: '#1a2d52', color: '#60a5fa' }}>
